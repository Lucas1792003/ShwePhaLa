-- ============================================================
-- Migration 005: Transactional refund / void approval RPCs
-- Moves approval writes out of the frontend and into atomic,
-- permission-checked Postgres functions.
-- Run AFTER 001-004. Idempotent (CREATE OR REPLACE).
-- ============================================================
  ...
  IF NOT EXISTS (
    SELECT 1
     ...
     WHERE schemaname = 'public'
       AND indexname = 'sales_shop_receipt_no_unique'
  ) THEN
    IF EXISTS (
      SELECT 1
        FROM sales
      ...
      RAISE NOTICE 'Skipped sales_shop_receipt_no_unique: duplicate (shop_id, receipt_no) rows exist.';
    ELSE
      CREATE UNIQUE INDEX sales_shop_receipt_no_unique
        ON sales (shop_id, receipt_no);
    END IF;
  END IF;
END;
$$;
...
CREATE OR REPLACE FUNCTION reject_refund_void_request(
  p_request_id text,
...
BEGIN
  ...
  v_perm := CASE WHEN v_request.type = 'VOID' THEN 'pos:void_sale' ELSE 'pos:refund' END;

  IF NOT app_can_for_shop(v_perm, v_request.shop_id) THEN
    RAISE EXCEPTION 'You are not permitted to reject this request';
  END IF;
  ...
  VALUES (
    v_audit_id, v_request.shop_id, v_user.id, 'APPROVAL_REJECTED',
    'Rejected ' || v_request.type || ' request. Reason: ' || COALESCE(p_reason, v_request.reason),
    'Refund', v_request.id, v_now
  );
  ...
  v_audit_logs := v_audit_logs || jsonb_build_array(jsonb_build_object(
    ...
    'message', 'Rejected ' || v_request.type || ' request. Reason: ' || COALESCE(p_reason, v_request.reason),
    'entityType', 'Refund',
    'entityId', v_request.id,
    'createdAt', v_now
  ));
...
REVOKE ALL ON FUNCTION approve_refund_request(text) FROM PUBLIC;
REVOKE ALL ON FUNCTION approve_void_request(text) FROM PUBLIC;
REVOKE ALL ON FUNCTION reject_refund_void_request(text, text) FROM PUBLIC;

GRANT EXECUTE ON FUNCTION approve_refund_request(text) TO authenticated;
GRANT EXECUTE ON FUNCTION approve_void_request(text) TO authenticated;
GRANT EXECUTE ON FUNCTION reject_refund_void_request(text, text) TO authenticated;
