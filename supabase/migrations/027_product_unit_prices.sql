-- ============================================================
-- Migration 027: Split product_units.price_mmk into
--                  sale_price_mmk (required, >= 0)
--                + purchase_price_mmk (nullable, >= 0)
--
-- Why:
--   Migration 026 stored a single `price_mmk` column on product_units.
--   That's enough to ring at POS but doesn't carry per-unit cost. With
--   multiple sellable units (Can, 6 Pack, Case) the per-unit purchase
--   price (case cost vs single-can cost) is independent of the sale
--   price and is needed for supplier debt and per-unit COGS later.
--
--   Spec: see docs/04-features-workflows.md → Products / Product Units.
--
-- Idempotent:
--   * Adds `sale_price_mmk` only if missing.
--   * Backfills `sale_price_mmk` from the legacy `price_mmk`.
--   * Drops the old `price_mmk` once the new column is populated.
--   * Adds nullable `purchase_price_mmk` if missing.
--   * Backfills `purchase_price_mmk` for default units from
--     `products.cost_mmk` (and only for rows where it is still NULL).
--
-- Safe to re-run.
-- ============================================================

ALTER TABLE product_units
  ADD COLUMN IF NOT EXISTS sale_price_mmk integer,
  ADD COLUMN IF NOT EXISTS purchase_price_mmk integer;

-- Copy the legacy price into sale_price_mmk where the new column is
-- still unset.
UPDATE product_units
   SET sale_price_mmk = price_mmk
 WHERE sale_price_mmk IS NULL
   AND price_mmk IS NOT NULL;

-- Belt-and-braces default for any row whose price is somehow null.
UPDATE product_units
   SET sale_price_mmk = 0
 WHERE sale_price_mmk IS NULL;

ALTER TABLE product_units
  ALTER COLUMN sale_price_mmk SET NOT NULL;

-- Replace the loose CHECK on the legacy column with checks on the new
-- columns. NOT VALID would keep historical rows out of scope, but we
-- want every row to satisfy the rule.
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM pg_constraint
     WHERE conrelid = 'product_units'::regclass
       AND conname = 'product_units_price_mmk_check'
  ) THEN
    ALTER TABLE product_units DROP CONSTRAINT product_units_price_mmk_check;
  END IF;
END;
$$;
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
     WHERE conrelid = 'product_units'::regclass
       AND conname = 'product_units_sale_price_nonneg'
  ) THEN
    ALTER TABLE product_units
      ADD CONSTRAINT product_units_sale_price_nonneg
      CHECK (sale_price_mmk >= 0);
  END IF;

  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint
     WHERE conrelid = 'product_units'::regclass
       AND conname = 'product_units_purchase_price_nonneg'
  ) THEN
    ALTER TABLE product_units
      ADD CONSTRAINT product_units_purchase_price_nonneg
      CHECK (purchase_price_mmk IS NULL OR purchase_price_mmk >= 0);
  END IF;
END;
$$;

-- Drop the legacy column once the new one is fully populated. Skipped
-- when an older version of this migration already removed it.
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
     WHERE table_name = 'product_units' AND column_name = 'price_mmk'
  ) THEN
    ALTER TABLE product_units DROP COLUMN price_mmk;
  END IF;
END;
$$;

-- Backfill `purchase_price_mmk` for the default unit of every product
-- using the legacy `products.cost_mmk`. Only touches NULL rows so
-- admin edits made through the new UI never get overwritten.
UPDATE product_units pu
   SET purchase_price_mmk = p.cost_mmk
  FROM products p
 WHERE pu.product_id = p.id
   AND pu.is_default = true
   AND pu.purchase_price_mmk IS NULL
   AND p.cost_mmk IS NOT NULL;

-- ============================================================
-- complete_sale RPC — switch to sale_price_mmk
-- ============================================================
...
BEGIN
  ...
  LOOP
    ...
    v_unit_price := COALESCE(NULLIF(v_item->>'unit_price_mmk', '')::integer, v_unit.sale_price_mmk);
    ...
    -- Tier pricing only applies to the default/base unit (see
    -- docs/04-features-workflows.md → Price tiers).
    IF v_unit.is_default THEN
      ...
      v_expected_price := COALESCE(v_expected_price, v_unit.sale_price_mmk);
    ELSE
