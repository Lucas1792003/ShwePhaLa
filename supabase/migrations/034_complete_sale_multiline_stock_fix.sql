-- ============================================================
-- Migration 034: complete_sale multi-line stock deduction fix
--
-- Bug (vs. migration 033):
--   When the SAME product appears on more than one cart line — e.g. a
--   different sellable unit (pk + ဩ) or a different price level of the
--   same product+unit — each line independently read `qty_before` from
--   the inventory table BEFORE any write, computed an absolute
--   `qty_after`, and the second loop wrote `UPDATE inventory SET
--   qty_base_units = qty_after` per line. Because both lines read the
--   same starting stock, their absolute writes overwrote each other and
--   only the LAST line's deduction stuck. Selling 1 pk (30) + 1 ဩ (10)
--   from 50 on-hand left 40 instead of 10.
--
-- Fix:
--   Keep a running on-hand value per product (`v_stock_running`) while
--   building the computed lines. The first line for a product reads +
--   locks the inventory row (FOR UPDATE) as before; subsequent lines for
--   the same product deduct from the previous line's result. This makes
--   every line's qty_before / qty_after cumulative, so the per-line
--   inventory writes AND the inventory_movements ledger are all correct,
--   and the final on-hand equals original - SUM(all line deductions).
--
-- Everything else is identical to migration 033.
-- ============================================================
